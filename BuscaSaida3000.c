...
*
* Entrada: Recebe um inteiro como argumento para identificar qual parte do menu printar.
...
{
    char interacao[25]; /*Variavel que recebe as Instrucoes enviadas pelo usuario.
                          Aloca-se uma string pois o usuario pode querer digitar uma palavra,
                          sendo assim o programa não irá dar crash se digitado uma palavra 'grande'.*/
...
int VerificacaoInicial( tPontoLabirinto *posicaoInicial , int *matrizLabirinto , const int *dimensao )
{
    ...
    for ( i = 0 ; i < *dimensao ; i++ )
    {
        for ( j = 0 ; j < *dimensao ; j++ )
        {
            ...
            {
                printf( "Algum elemento do labrinto é diferente de 0 ou 1, corrija isto!\n\n" );
    ...
    if ( *dimensao < 0 )
    {
        printf( "Dimensao do labirinto menor que zero, corrija isto!\n\n" );
        return 0;
    }

    else if ( posicaoInicial->x < 0 || posicaoInicial->y < 0 )
    {
        printf( "Uma das coordenadas é menor que zero, corrija isto!\n\n" );
    }

    else if ( ( matrizLabirinto[ posicaoInicial->x * *dimensao + posicaoInicial->y ] == 0 ) ) //Se estiver em uma passagem obstruida
    {
        printf( "Iniciou no labirinto em cima de uma passagem obstruida, corrija isto!\n\n" );
        return 0; //Se sim retorna falso
    }

    else if ( ( ( posicaoInicial->x + 1 ) > *dimensao ) || ( ( posicaoInicial->y + 1 ) > *dimensao )  ) //Se estiver fora da matriz
    {
        printf( "Iniciou no labrinto do lado de fora, corrija isto!\n\n" );
        return 0; //Se sim, retorna falso
    }
